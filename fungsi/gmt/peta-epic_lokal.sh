...
awk '{ print $1+0.07,$2,$3,$4,$5,$6,$7}' $kota_besar_malut | pstext -R -JM -O -K >> $psfile
...
psbasemap -J -R$REGION -B$AXIS --MAP_LABEL_OFFSET=0.2 --FONT_ANNOT_PRIMARY=9p --FONT_LABEL=11p,Heveltica --MAP_FRAME_TYPE=inside --MAP_FRAME_WIDTH=0.1p -P -K -O -V -Lf$skala/1/50+lKM  >> $psfile
...
awk '{ print $4,$5}' fungsi/gmt/param.txt | pstext -R1/10/1/10 -JX4 -F+cTL+f9.5,Bookman-Demi,white  -P -O -K -Y-0.45 >> $psfile
awk '{ print $6,$7,$8,$9,$10}' fungsi/gmt/param.txt | pstext -R1/20/1/10 -JX5 -F+cTL+f9.3,Bookman-Demi,white  -P -O -X-0.07 -Y-2.9  -K  >> $psfile
